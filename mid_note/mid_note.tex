...
%%%%%%%%%%%%%%%Title的資訊%%%%%%%%%%%%%%%
\title{Signals and Systems Midterms Notes} %標題
\author{LeoMao} %作者
\date{\today} %日期
...
\begin{minipage}{0.5\textwidth}
  ...
  \begin{enumerate}[noitemsep,nolistsep]
    \item memoryless: output at given time only depends on input at the
      same time.
    \item causality: 和未來無關
    \item invertibility: distinct inputs lead to distinct outputs.
    \item stabiliy: bounded inputs lead to bounded outputs.
    \item time invariance: $x(t) \to y(t) \Rightarrow x(t-t_0) \to
      y(t-t_0)$.
  \end{enumerate}
  對 LTI 而言
  \begin{enumerate}[noitemsep,nolistsep]
    \item causality 即 $h(t) = 0$ for $t < 0$ ($h[n] = 0$ for $n < 0$).
    \item stable $\Leftrightarrow$ impulse response absolutely
      integrable/summable.
  \end{enumerate}
  convolution: $\int\limits_{-\infty}^{\infty} x(\tau)y(t-\tau)d\tau
  \quad \sum\limits_{k=-\infty}^{\infty} x[k]y[n-k]$ \\
  Fourier Transform/Series easy properties: linearity, conjugation,
  time reversal.\\
  $\delta(at) = \frac{1}{a}\delta(t)$ \\
  $Ev\{f(x)\} = \frac{f(x) + f(-x)}{2} \quad
  Od\{f(x)\} = \frac{f(x) - f(-x)}{2}$
\end{minipage}
